...
### Changes in version 1.3.8 (2021-09-06)
#### BUG FIXING
...
#### MAJOR UPDATES
  * The package dependencies that are used only in the interactive app *per se*
    ...
    of them if you plan to only use the package's "core" functions, such as
    `DDplot()` or `ItemAnalysis()`.
  * `startShinyItemAnalysis()` now checks for dependencies required for its
     proper run and offers to install them if any of them is not available.
...
#### MINOR UPDATES
  ...
  * R functions and data manuals were updated
